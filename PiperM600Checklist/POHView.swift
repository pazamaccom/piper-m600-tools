...
struct POHView: View {
    @StateObject private var model = POHViewModel()
    ...
    var body: some View {
        GeometryReader { geo in
                ...
                .overlay {
                    if model.document == nil {
                        VStack(spacing: 12) {
                            ...
                            Text("Open Settings to load a PDF or download the default POH.")
                                .font(.subheadline)
            ...
            .onAppear {
                ...
                if POHIndexCache.consumeClearedFlag() {
                    Task {
                        await model.rebuildIndex(forceReload: true)
                    }
                }
            }
            .onReceive(NotificationCenter.default.publisher(for: POHIndexCache.didClearNotification)) { _ in
    ...
    private var headerView: some View {
        VStack(spacing: 4) {
            Text("Pilot's Operating Handbook")
    ...
    private var searchBar: some View {
        HStack(spacing: 10) {
            ...
            TextField("Search POH", text: $model.query)
...
@MainActor
final class POHViewModel: ObservableObject {
    @Published var query: String = "" {
        didSet {
            scheduleSearch()
        }
    }
    @Published var results: [POHSearchHit] = []
    ...
    @Published var outlineItems: [POHOutlineItem] = []
    @Published var indexDetail = ""

    private(set) var document: PDFDocument?
    private var searchTask: Task<Void, Never>?
    private var index: POHSearchIndex?
    ...
    var resultsSummary: String {
        guard document != nil else { return "Load a POH PDF in Settings." }
    ...
    func onAppear() {
        ...
        document = POHDocumentProvider.loadDocument()
        if let document {
            outlineItems = POHOutlineItem.build(from: document)
        }
        Task {
    ...
    private func buildIndexIfNeeded() async {
        guard let pdfURL = POHDocumentProvider.documentURL else { return }
        if document == nil {
            document = POHDocumentProvider.loadDocument()
        }
        ...
        if let cached = POHSearchIndex.loadIfValid(from: pdfURL) {
    ...
    private func processNextIndexBatch(pdfURL: URL) {
        ...
        for pageIndex in indexingCurrentPage..<end {
            ...
            let tokens = Set(POHSearchIndex.tokens(from: indexingPageTexts[pageIndex]))
            for token in tokens {
    ...
    private func finishIndexing(pdfURL: URL) {
        if let built = POHSearchIndex.build(from: indexingPageTexts, tokenIndex: indexingTokenIndex) {
...
private enum POHDocumentProvider {
    static var documentURL: URL? {
        guard DocumentStorage.exists(.poh) else { return nil }
        return DocumentStorage.localURL(for: .poh)
    }
...
struct POHSearchHit: Identifiable {
    let id = UUID()
    let pageIndex: Int
    let range: NSRange
    let excerpt: String
...
struct POHOutlineItem: Identifiable {
    let id = UUID()
    let title: String
    let pageIndex: Int
    let children: [POHOutlineItem]

    static func build(from document: PDFDocument) -> [POHOutlineItem] {
        guard let outlineRoot = document.outlineRoot else { return [] }
        var items: [POHOutlineItem] = []

        func buildNode(_ outline: PDFOutline) -> POHOutlineItem? {
            ...
            guard !title.isEmpty else {
                return children.isEmpty ? nil : POHOutlineItem(title: "Section", pageIndex: pageIndex, children: children)
            }

            return POHOutlineItem(title: title, pageIndex: pageIndex, children: children)
        }
...
struct POHTableOfContentsView: View {
    let items: [POHOutlineItem]
        ...
        List {
            if items.isEmpty {
                ...
                ForEach(items) { item in
                    NavigationLink {
                        POHSectionChaptersView(title: item.title, items: item.children, onSelect: onSelect)
                    } label: {
                        HStack {
...
struct POHSectionChaptersView: View {
    let title: String
    let items: [POHOutlineItem]
    let onSelect: (Int) -> Void
...
private struct POHSearchIndex: Codable {
    let version: Int
    let pageCount: Int
    ...
    static func loadIfValid(from pdfURL: URL) -> POHSearchIndex? {
        guard let attributes = try? FileManager.default.attributesOfItem(atPath: pdfURL.path),
              let fileSize = attributes[.size] as? UInt64 else {
            return nil
        }

        guard let data = try? Data(contentsOf: POHIndexCache.url),
              let cached = try? JSONDecoder().decode(POHSearchIndex.self, from: data) else {
            return nil
        }
    ...
    func save(to pdfURL: URL) {
        ...
        try? data.write(to: POHIndexCache.url, options: [.atomic])
    }
    ...
    static func build(from pageTexts: [String], progress: @escaping (Double) -> Void) -> POHSearchIndex? {
    ...
    static func build(from pageTexts: [String], tokenIndex: [String: Set<Int>]) -> POHSearchIndex? {
        let total = pageTexts.count
        guard total > 0 else { return nil }
...
private struct POHFullScreenView: View {
